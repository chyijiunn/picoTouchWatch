...
# ----------------------------
ACC_SCALE = 1500.0  # bigger => faster response (was 880)
DAMP = 0.995
FRICTION = 0.010
DEAD = 0.05
...
DT_MAX = 0.06  # smaller max dt reduces tunneling through walls
...
MAZE_R_INNER = 24
MAZE_R_OUTER = 112
MAZE_THICK   = 5   # thicker walls reduce slipping through corners
...
def add_arc_segments(walls, r, a0, a1, thick, seg_len=18):
    """Arc -> polyline segments with INT-continuity to avoid pixel gaps."""
    da = a1 - a0
    if da < 0:
        da += 2 * math.pi
    arc_len = abs(r * da)
    n = max(2, int(arc_len / seg_len))
    # Compute points and round ONCE, chaining endpoints to guarantee continuity
    aa = a0
    x0f, y0f = polar_to_xy(r, aa)
    x0 = int(x0f); y0 = int(y0f)
    for k in range(1, n+1):
        aa = a0 + da * (k / n)
        x1f, y1f = polar_to_xy(r, aa)
...
# ----------------------------
WALLS = []
ball = {"x": float(CX), "y": float(CY), "vx": 0.0, "vy": 0.0, "r": 5, "c": c_ball}
...
while True:
    ...
    # --- Physics integrate ---
    # Sub-stepping: prevents tunneling when dt is large (fast movement)
    steps = int(dt / 0.02) + 1  # keep each step <= ~20ms
    if steps > 6:  # cap to avoid heavy CPU if the loop stalls
        steps = 6
    h = dt / steps
    ...
    for _ in range(steps):
        ...
        # Collisions: do two passes for stability at corners
        collide_with_outer_circle(ball)
        for wall in WALLS:
            collide_ball_with_wall(ball, wall)
        collide_with_outer_circle(ball)
        # second pass helps when ball overlaps multiple walls in one step
        for wall in WALLS:
            collide_ball_with_wall(ball, wall)
        collide_with_outer_circle(ball)
